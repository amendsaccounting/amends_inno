...
def _execute(filters=None, additional_table_columns=None, additional_query_columns=None):
	...
	for d in item_list:
		...
		foc_quantity=calculate_item_stock_qty()
		foc_quantity = [each_item["foc"] for each_item in foc_quantity if each_item["name"]==d.name][0] if [each_item["foc"] for each_item in foc_quantity if each_item["name"]==d.name] else 0
		row.update({
			'debit_to': d.debit_to,
			...
			'stock_qty': d.stock_qty if foc_quantity == 0 else 0,
