# Copyright (c) 2022, Amends Accouting and contributors
...
def _execute(filters, additional_table_columns=None, additional_query_columns=None):
	...
	foc_quantity=get_total_foc_count(invoice_list)
	data = []
	for inv in invoice_list:
		...
		# foc_quantity = list(set(invoice_cc_wh_map.get(inv.name, {}).get("foc_quantity", [])))
		# print(foc_quantity,"///////")
		...
		row = {
			...
			"foc_quantity":[each_item["foc"] for each_item in foc_quantity if each_item["parent"]==inv.name][0] if [each_item["foc"] for each_item in foc_quantity if each_item["parent"]==inv.name] else 0
		}
		print(row["foc_quantity"],";;;;;;;;;;;;;;;;;;;")
		...
		row.update({
			...
			# "foc_quantity":row["foc_quantity"][0] if row["foc_quantity"] else 0,
...
def get_columns(invoice_list, additional_table_columns):
	...
	total_columns = [
		...
		},
		{
			...
			"label": _("FOC Quantity"),
			"fieldname": "foc_quantity",
			"fieldtype": "int",
			# "options": 'int',
			"width": 120
		},
...
def get_invoices(filters, additional_query_columns):
	...
	return frappe.db.sql("""
		select `tabSales Invoice`.name, posting_date, debit_to, project, `tabSales Invoice`.customer,
		customer_name,`tabSales Invoice`.owner, remarks, territory, tax_id, customer_group,
		base_net_total, base_grand_total, base_rounded_total, outstanding_amount, total_qty,
		is_internal_customer, represents_company, company {0}
		from `tabSales Invoice` 
		where `tabSales Invoice`.docstatus = 1 %s order by posting_date desc, name desc""".format(additional_query_columns or '') %
...
def get_total_foc_count(invoice_list):
	total_count = {}
	if invoice_list:
